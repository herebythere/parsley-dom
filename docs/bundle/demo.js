...
const parsleyNoStackDependenices =
  `It does not rely on DOM Templates\nor JSX and is fully capable of being ported to other languages.`;
...
const countDisplayMemoryDemoCode =
  `const descendants = [\n  countDisplay(0),\n  countDisplay(0),\n  countDisplay(0),\n];`;
const updateDisplayMemoryDemoCode =
  `// typescript\nconst updateDescendants = (count: number) => {\n  let index = 0;\n  while (index < descendants.length) {\n    const descendant = descendants[index];\n    const value = Math.pow(2, index * count);\n    descendant.update(value);\n\n    index += 1;\n  }\n};`;
const counterWithSavedChildrenDemoCode =
...
const parsleyLifecycle =
  `Parsley-DOM has a three-part lifecycle:\nconnect, update, and disconnect.`;
const counterChunk = counterFactory();
