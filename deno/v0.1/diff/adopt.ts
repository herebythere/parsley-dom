...
function adoptNodes<N>(
	...
	while (survIndex < delta.survivedIndexes.length) {
		...
		for (let descArrayIndex = 0; descArrayIndex < descArrayLength; descArrayIndex++) {
			...
			const descLength = Math.max(prevDescIndexes.length, descIndexes.length);
			const prevSource = prevRender.sources[prevDescNodeIndex];
			const source = prevRender.sources[descNodeIndex];
